...
`index.html?promo=1` 是内置的 180 秒项目宣传片，用来直接录成投稿视频。
...
八个场次：片头（代码窗口打字机）→ 身份（单文件 · 零依赖）→ 界面（粘贴与语言识别）→ 模版巡礼（八个模版逐个套用）→ 出现方式 × 镜头运动 → 关键帧编辑器（含拖动写入关键帧的演示）→ 技术特征 → 片尾。

- 中文字幕烧录在画面里，附模拟光标与界面元素高亮环（指哪打哪，元素本身不动）。
- 屏幕底部控制条：播放 / 暂停、重播、进度拖拽、全屏、开始录制；按 `H` 隐藏控件，`空格` 播放 / 暂停。
- **录制**：点「开始录制」→ 在浏览器弹窗里选「此标签页」→ 自动从 0 秒播满 180 秒并下载 WebM（VP9 / 24 Mbps）。因为界面是真实 DOM 而不是画布，所以走系统录屏 `getDisplayMedia`，不走 `canvas.captureStream`。
