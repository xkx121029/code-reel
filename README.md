...
### 2026-09-20 · 右键拖动不再被浏览器手势抢走

- **问题**：右键拖动会触发浏览器自己的右键菜单 / 右键手势，拖到一半被接管，位移全丢（只剩一个 `pointercancel`）。
- **原因**：右键按下时没有阻止默认行为 —— 浏览器的右键菜单与右键手势都是从这一次默认行为起手的；而 `contextmenu` 只在画布上被拦，拖到画布外松手时菜单会挂在鼠标底下那个元素上（pointer capture 管不到它的重定向）。
- **修复**：右键 `pointerdown` 直接 `preventDefault()`（只吞右键 —— 左键吞了会连带掐掉兼容鼠标事件，双击复位就没了）；拖动期间在**整个文档**范围内压住 `contextmenu`，松手即恢复；再补上 `dragstart` 拦截与画布的 `user-select / user-drag: none`，避免拖动被当成原生选择或原生拖拽。
- 需要说明：若浏览器开启了**内置鼠标手势**或装了鼠标手势扩展（在浏览器设置里，而非页面层），页面无法完全阻止。这类情况请在浏览器设置里关掉手势，平移则改用时间轴手动打点。
